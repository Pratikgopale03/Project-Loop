        ...
        <div className="lg:col-span-8 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <select
                  ...
                  className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs py-1.5 px-2.5 text-slate-700 dark:text-slate-300 focus:outline-none cursor-pointer transition-colors"
                >
                  <option value="ALL" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">All Sentiments</option>
                  <option value="POS" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Positive</option>
                  <option value="NEU" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Neutral</option>
                  <option value="NEG" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Negative</option>
                </select>
              </div>
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  <option value="REVIEWED" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Reviewed</option>
                  <option value="ACTIONED" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Actioned</option>
                </select>
              </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex flex-col sm:flex-row justify-between items-center py-3 gap-3 border-t border-slate-200 dark:border-slate-800/80 mt-2">
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                    Showing <strong className="text-slate-800 dark:text-slate-200">{(page - 1) * 5 + 1}</strong> - <strong className="text-slate-800 dark:text-slate-200">{Math.min(page * 5, total)}</strong> of <strong className="text-slate-800 dark:text-slate-200">{total}</strong> items
                  </span>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <button
                      ...
                      disabled={page === 1}
                      className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 bg-white dark:bg-slate-900/40 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 disabled:opacity-30 transition"
                      title="Previous Page"
                    >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                      ...
                      .map((pNum, idx, array) => (
                        <div key={pNum} className="flex items-center gap-1">
                          {idx > 0 && array[idx - 1] !== pNum - 1 && (
                            <span className="text-xs text-slate-400 dark:text-slate-500 px-1 font-bold">...</span>
                          )}
                          <button
                            onClick={() => setPage(pNum)}
                            className={`min-w-[32px] h-8 px-2.5 rounded-lg text-xs font-bold transition border ${
                              page === pNum
                                ? "bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-600/20"
                                : "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-200"
                            }`}
                          >
                            {pNum}
                          </button>
